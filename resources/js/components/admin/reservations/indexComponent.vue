...
export default {
    ...
    setup() {
        const reservations = ref([]); // Make members reactive to store fetched data

        const fetchReservations = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_APP_URL}/api/reservations`);
                if (response.data && response.data.data && response.data.data.data[0]) {
                    console.log('response', response.data.data.data)
                    reservations.value.push(...response.data.data.data);
                    console.log("Fetched members reservations data:", reservations.value);
                } else {
                    console.log("No members reservations found.");
                }
            } catch (error) {
                console.error("Error fetching members data:", error);
            }
